...
| Estado visual en tienda | `ShopRequirementsUI` en `EndScene/ShopCanvas/.../ShopContent/Requirements`: stock/mínimo de carne y carbón, cuánto falta, y la racha. Verde/rojo según se cumpla |
...
| 30 | **La fuente de la tienda (`Electronic Highway Sign SDF`) es Dynamic y no tiene todos los glifos.** `—` y `→` **no existen** en el TTF y salen como cuadrado (ya pasa en los labels viejos de `ShopNextButtonUI`); los acentos sí se agregan solos al atlas. Al escribir texto nuevo de tienda, evitar signos exóticos |
