...
#### UI de tienda (capa uGUI activa) — `Shop/*UI.cs` · `Prefabs/UI/ShopCanvas.prefab`

**Un solo prefab para las dos escenas.** Desde el rediseño del 2026-09-25 el canvas de la tienda es
`Prefabs/UI/ShopCanvas.prefab`, instanciado en `EndScene` y en `ShopTutorial`. Cada instancia solo pisa la
referencia `shop` de los scripts (cada escena tiene su propio `ShopSystem`: catálogo y `enforceRunMinimums`
...
[ NamePlate "PARRILLA 40" | "Hoy vinieron N clientes"            MoneyPlate "$1.000" ]  ← caja de la persiana (Header)
            [ 1. CARBÓN ][ 2. CARNES ][ 3. MEJORAS ][ 4. TOPPINGS ]                     ← Tabs
            [ línea de ayuda de la tab (Nunito) ]                                        ← SubHeader
            [ tarjeta ][ tarjeta ][ tarjeta ][ tarjeta ]   (4 columnas, scroll vertical)  ← GridArea
...
- **Botones (tabs, Siguiente, Comprar, −/+):** `ColorTint` marcado (hover `0.78`, apretado `0.62`, deshabilitado
  `0.5` con alfa `0.85`) + `MenuButtonHover` (agranda al hover: `1.06`, el −/+ `1.1`). `Navigation = None`: si no,
  el click deja el botón *Selected*, y ese estado le gana al hover y lo apagaba. En la tarjeta,
...
| Estado visual en tienda | `ShopRequirementsUI` en `ShopCanvas.prefab/.../ShopContent/Requirements` (abajo a la izquierda): encabezado fijo "PARA ARRANCAR MAÑANA", stock/mínimo de carne y carbón, a la derecha "LISTO PARA ARRANCAR" o cuánto falta, y debajo la racha (`"NOCHES SEGUIDAS CON STRIKES: N / M"`), que **solo aparece con racha > 0**. Verde/rojo según se cumpla. Fuente **Bungee** (tiene acentos; ver nota 30) sobre la chapa oscura `Boton Continuar` en `Sliced` |
...
| 8 | `Shop` tiene **dos capas de UI paralelas**. La activa es **`*UI` (uGUI, en `EndScene` y `ShopTutorial`)**; la capa `*2D` sigue en la escena dentro del prefab `ShopRoot` pero **desactivada** (`m_IsActive: 0`) — al tocar la tienda, editar los `*UI` y el prefab `ShopCanvas` (nota 34) |
...
| 14 | **Una bolsa de carbón trae `unitsPerBag` unidades**: el default del código es `1`, pero **`CoalData.asset` lo pisa con `10`** — comprar 1 en la tienda suma 10 al cooler (`Cooler.Add(coal, unitsPerBag × qty)`). La celda lo aclara en el nombre y la descripción (ver `ShopItemCellUI`), y el stock del carbón se muestra en **unidades**, igual que el header (`GetTotalCoalUnits()`). `GetSuggestedCoalBags()` y `CartCoalBags()` razonan en bolsas |
...
| 30 | **`Electronic Highway Sign SDF` no tiene acentos ni `—` / `→`** y los dibuja como cuadrado (ya pasa en los labels viejos de `ShopNextButtonUI` y en `CantCarbones`). **`Bungee-Regular SDF` y `Nunito-Regular SDF` sí tienen los acentos**: para texto nuevo de tienda usar esas dos, y evitar igual los signos exóticos (`·`, `—`, `→`). **`BungeeHairline-Regular SDF` no sirve para texto chico**: es de trazo "pelo" y por debajo de ~20 pt se ve cortado. Desde el rediseño la tienda ya no la usa: solo Bungee y Nunito. Para texto de cuerpo, Nunito |
